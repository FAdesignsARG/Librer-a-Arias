...
/* ==========================================================================
   PROMOCIONES
   Desde el 17/9/2026 la promo es una sola: un porcentaje por comprar en la
   web (webPercent) y una aclaración. Mismo patrón que #settingsForm: el
   dialog precarga settings.promos, se edita acá, y al guardar se
...
$('#btnPromos').addEventListener('click', () => {
  $('#pWebPercent').value = settings.promos?.webPercent ?? 10;
...
$('#promosForm').addEventListener('submit', async (e) => {
  ...
  const raw = $('#pWebPercent').value.replace(/\D/g, '');
  const webPercent = Number(raw) || 0;
  if (webPercent > 100) return toast('El descuento tiene que estar entre 0 y 100.');
